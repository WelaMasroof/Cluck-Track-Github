        ...
        private void gettotaleggs()
        {
            try
            {
                using (SqlConnection sqlConnection = new SqlConnection(ConnectionString))
                {
                    ...
                    string Query = "SELECT SUM(amount) FROM Sales where category='Eggs'";
                    ...
                    using (SqlDataAdapter adapter = new SqlDataAdapter(Query, sqlConnection))
                    {
                        ...
                        if (dataTable.Rows.Count > 0 && dataTable.Rows[0][0] != DBNull.Value)
                        {
                            eggs.Text = dataTable.Rows[0][0].ToString();
                        }
                        else
                        {
                            eggs.Text = "0"; // Assuming you want to display "Rs 0" if no sales are found
                        }
                    }
                }
            }
